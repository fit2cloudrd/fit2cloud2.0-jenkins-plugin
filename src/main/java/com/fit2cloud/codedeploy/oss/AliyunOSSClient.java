...
import com.fit2cloud.codedeploy.CodeDeployException;
import com.fit2cloud.codedeploy.Utils;
...
public class AliyunOSSClient {
    ...
    public static boolean validateAliyunAccount(
            final String aliyunAccessKey, final String aliyunSecretKey) throws CodeDeployException {
        try {
            OSSClient client = new OSSClient(aliyunAccessKey, aliyunSecretKey);
            client.listBuckets();
        } catch (Exception e) {
            throw new CodeDeployException("阿里云账号验证失败：" + e.getMessage());
        }
        return true;
    }
